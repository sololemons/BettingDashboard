...
<template>
  <div class="flex flex-col justify-between items-start gap-4 p-4">
    ...
    <div class="relative flex flex-col bg-stone-200 rounded-2xl border border-gray-200 p-6 w-full 
      sm:w-11/12 md:w-full">
      <div class="grid grid-cols-1 sm:grid-cols-4 lg:grid-cols-4 gap-6">

        <div class="bg-orange-400 h-32 sm:h-40 md:h-44 lg:h-48 rounded-xl shadow-md flex items-center justify-center 
          transform hover:scale-95 transition border border-gray-300">
          <p class="text-gray-700 font-semibold text-lg text-center"><span>Total Bets:</span><span
              class="font-extrabold text-lg text-green-400">{{ totalBetsPlaced }}</span></p>
        </div>

        <div class="bg-yellow-300 h-32 sm:h-40 md:h-44 lg:h-48 rounded-lg shadow-md flex items-center justify-center 
          transform hover:scale-95 transition border border-gray-300">
          <p class="text-gray-700 font-semibold text-lg text-center"><span>Total Active Users:</span><span
              class="font-extrabold text-lg text-green-400">{{ totalActiveUsers }}</span></p>
        </div>

        <div class="bg-red-400 h-32 sm:h-40 md:h-44 lg:h-48 rounded-lg shadow-md flex items-center justify-center 
          transform hover:scale-95 transition border border-gray-300">
          <p class="text-gray-700 font-semibold text-lg text-center"><span>Total Revenue: </span><span
              class="font-extrabold text-lg text-green-400">{{ totalRevenue }}</span></p>
        </div>
