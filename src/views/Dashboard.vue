...
<template>
  <div class="flex h-screen bg-gray-10">
    <aside
      class="w-60 bg-blue-900 text-white flex flex-col p-6 space-y-6 absolute h-full md:relative md:block md:h-screen z-50 transition-transform transform md:translate-x-0"
      :class="{ '-translate-x-full': !isSidebarOpen }"
    >
      <h2 class="text-2xl font-extrabold text-center text-white bg-black p-2">
        SoloBETS
      </h2>
      <nav class="flex-1">
        <ul class="space-y-4">
          ...
          <div class="mt-auto relative">
            ...
            <ul
              v-if="dropdownOpen"
              class="absolute bg-white shadow-md rounded mt-1 w-40 text-black"
            >
              <li>
                <router-link
                  to="/viewAdmins"
    ...
    <button
      @click="toggleSidebar"
      class="fixed top-4 left-4 md:hidden bg-blue-900 text-white p-2 rounded z-50"
    >
      ☰
    </button>
